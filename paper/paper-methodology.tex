This section outlines the experimental design anbd the testbed used.
...
parameter sizes (smallest: 9B, largest from leading frontier models). Models are queried twice with reasoning disabled and enabled, unless the API does not
allow for disabling of reasoning mode. See Appendix "\nameref{app:llms}" on page \pageref{tab:models} for the full list of models.
...
Three states are declared: weather\_state (it is either raining or not raining), 
street\_state (the street is either dry or wet), and traction\_state 
...
Two contexts $\omega$ and $\nu$ let a weather\_state w be raining and a street\_state s be wet.

A constraint is added to explicitly ask the model to output only symbolic notation, and only the conclusion.
Furthermore, a "No\_Prose" constraint is given as an additional hint.
